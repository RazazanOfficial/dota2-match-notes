...
function doGet(e) {
  ...
  try {
    ...
    const steamId = normalizeSteamId_(e.parameter.steamId);
    const db = readDatabase_();
    const user = db.users[steamId];
    if (!user) throw new Error("بازیکنی با این آیدی استیم پیدا نشد");
...
function handlePost_(request) {
  if (request.action === "auth") {
    return authenticate_(request.steamId, request.password);
  }
...
function authenticate_(rawSteamId, rawPassword) {
  const steamId = normalizeSteamId_(rawSteamId);
...
function saveDay_(request) {
  const steamId = normalizeSteamId_(request.steamId);
...
function normalizeSteamId_(value) {
  const steamId = String(value || "").replace(/\s+/g, "");
  if (!STEAM_ID_PATTERN.test(steamId)) throw new Error("SteamID64 معتبر وارد کنید");
  return steamId;
}
