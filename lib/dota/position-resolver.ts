...
function evidence(player:Raw,manualPosition:number|null):PositionCandidate|null{
  ...
  if(manualPosition&&Number.isInteger(manualPosition)&&manualPosition>=1&&manualPosition<=5){add("manual","Position توسط کاربر تأیید شده",100,[manualPosition]);return{slot,scores,evidence:items,source:"manual",preferred:manualPosition};}
...
function resolveTeam(candidates:PositionCandidate[]){
  ...
  const resolved=new Map<number,{position:number|null;candidate:PositionCandidate;confidence:number}>(),fixed=candidates.filter((candidate)=>candidate.source==="manual"),open=candidates.filter((candidate)=>!fixed.includes(candidate));
...
export function resolveMatchPositions(params:{players:Raw[];positionOverrides?:Record<string,number>|null;profileSlot:number|null;profileAssignedPosition:number|null}){
  const candidates=params.players.flatMap((player)=>{const slot=num(player.player_slot);if(slot===null)return[];const candidate=evidence(player,params.positionOverrides?.[String(slot)]??null);return candidate?[candidate]:[];});
