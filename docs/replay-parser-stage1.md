...
این فهرست ممکن است در یک Match ناقص یا غایب باشد. کاربر می‌تواند آن را
در فرم Match تغییر دهد؛ تغییر دستی، حتی حذف همهٔ Banها، ذخیره می‌شود و
بر نسخهٔ خودکار اولویت دارد. دکمهٔ بازگشت، اولویت دستی را حذف می‌کند و
پس از ذخیره، Banهای موجود OpenDota دوباره نمایش داده می‌شوند. دریافت
Ban از local replay تا وقتی دقت draft events در چند Match ثابت نشده
فعال نیست.
