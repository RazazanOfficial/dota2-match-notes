...
function Summary({ player,players,duration,confirmPositions }: { player: MatchPlayerAnalysis;players:MatchPlayerAnalysis[];duration:number;confirmPositions?: (updates:Record<string,number>)=>void }) {
  ...
  return <div className="analysis-summary">{swapPlayers.length>=2&&<RoleSwapReview players={swapPlayers} confirm={confirmPositions}/>}<DomainProfile player={player} duration={duration}/><div className="analysis-overview-grid">
...
function DomainProfile({player,duration}:{player:MatchPlayerAnalysis;duration:number}){const domains=calculatePerformanceDomains(player.benchmarks,duration,player.position);const score=playerScore(player,duration);return <section className="performance-domain-profile"><header className="performance-score-hero"><div className="performance-score-title"><CircleGauge/><span><strong lang="en">Performance Score</strong></span></div><div className={`performance-hero-frame is-${performanceTone(score)}`}><img src={widePortrait(player)} alt={player.heroName}/></div><div className="performance-hero-meta"><strong lang="en" dir="ltr">{player.heroName}</strong><hr/><span><img src={positionIcon(player.position)} alt=""/><b lang="en" dir="ltr">Position {player.position??"?"}</b></span></div><ScoreRing score={score}/></header><div className="performance-domain-grid">{domains.map((item)=>{const copy=DOMAIN_COPY[item.key];return <article key={item.key} className={`is-${item.score==null?"unavailable":performanceTone(item.score)}`}><header><span><small lang="en">{item.label}</small><b>{copy.fa}</b></span><strong className="latin-numerals" lang="en" dir="ltr"><b>{item.score??"—"}</b><small>/100</small></strong></header><p>{copy.detail}</p><footer><span>{item.metricCount?`${en.format(item.metricCount)} معیار قابل محاسبه`:"داده کافی نیست"}</span><i><em style={{width:`${item.score||0}%`}}/></i></footer></article>})}</div></section>}
...
function playerScore(player: MatchPlayerAnalysis, duration: number) { return calculatePerformanceScore(player.benchmarks, duration,player.position); }
...
function ScoreRing({ score }: { score: number }) { const color = performanceTone(score); return <div className={`analysis-score-ring is-${color}`} style={{ "--score": `${score * 3.6}deg` } as CSSProperties}><span lang="en" dir="ltr"><b>{score}</b><small>/100</small></span></div>; }
