...
import { Activity, AlertTriangle, ArrowDown, ArrowRightLeft, ArrowUp, BarChart3, Check, ChevronLeft, ChevronRight, CircleGauge, Clock3, Eye, EyeOff, Info, Lightbulb, MapPinned, Minus, RefreshCw, ShieldAlert, Sparkles, TrendingUp, UsersRound, X } from "lucide-react";
...
export default function MatchAnalysisPanel({ match, active, canRequestAnalysis = false, onPositionOverrides }: { match: Match; active: boolean; canRequestAnalysis?: boolean; onPositionOverrides?: (updates:Record<string,number>) => void }) {
  ...
  return <section className="match-analysis">
    ...
    <div className="analysis-player-dock"><PlayerStrip players={analysis.players} selected={player.playerSlot} select={setSlot} />{onPositionOverrides&&<PositionSwapMenu player={player} players={analysis.players} request={requestPositions}/>}</div>
...
function LaneImpactPanel({player,players}:{player:MatchPlayerAnalysis;players:MatchPlayerAnalysis[]}){
  ...
  const label=lane.assessment==="ahead"?"برتری در Lane":lane.assessment==="behind"?"عقب‌افتادگی در Lane":"Lane نزدیک";
  return <section className={`lane-impact-panel is-${lane.assessment}`}><header><span><Activity/></span><div><small>LANE IMPACT · FIRST 10 MINUTES</small><strong>{label}</strong><p>{lane.roleGroup==="support"?"برای Support، XP، مرگ، منابع مصرفی و Vision اولیه بررسی شده‌اند؛ LH پایین جریمه نشده است.":"برای Core، NW، XP، LH، Deny و مرگ‌های ده دقیقه اول بررسی شده‌اند."}</p></div>{opponent&&<aside><img src={portrait(player)} alt=""/><b>VS</b><img src={portrait(opponent)} alt=""/><span lang="en">{opponent.heroName}</span></aside>}</header><div>{lane.evidence.map((item)=><span key={item}>{item}</span>)}</div><footer>اطمینان تحلیل: <b>{lane.confidence==="high"?"بالا":lane.confidence==="medium"?"متوسط":"محدود"}</b> · این Context وارد Percentile جهانی نمی‌شود.</footer></section>;
}
...
function DomainProfile({player,duration}:{player:MatchPlayerAnalysis;duration:number}){const scoreData=globalScoreMetrics(player);const domains=calculatePerformanceDomains(scoreData,duration,player.position);const score=playerScore(player,duration);const scoreTone=score===null?"unavailable":performanceTone(score);return <section className="performance-domain-profile"><header className="performance-score-hero"><div className="performance-score-title"><CircleGauge/><span><strong lang="en">Performance Score</strong></span></div><div className={`performance-hero-frame is-${scoreTone}`}><img src={widePortrait(player)} alt={player.heroName}/></div><div className="performance-hero-meta"><strong lang="en" dir="ltr">{player.heroName}</strong><hr/><span><img src={positionIcon(player.position)} alt=""/><b lang="en" dir="ltr">Position {player.position??"?"}</b></span></div><ScoreRing score={score}/></header><div className="performance-domain-grid">{domains.map((item)=>{const copy=DOMAIN_COPY[item.key];return <article key={item.key} className={`is-${item.score==null?"unavailable":performanceTone(item.score)}`}><header><span><small lang="en">{item.label}</small><b>{copy.fa}</b></span><strong className="latin-numerals" lang="en" dir="ltr"><b>{item.score??"—"}</b><small>/100</small></strong></header><p>{copy.detail}</p><footer><span>{item.metricCount?`${en.format(item.metricCount)} معیار قابل محاسبه`:"داده کافی نیست"}</span><i><em style={{width:`${item.score||0}%`}}/></i></footer></article>})}</div></section>}
...
function MetricCard({ entry }: { entry: MatchBenchmarkMetric }) {
  const info = presentation(entry); const direction = info.direction === "lower" ? "مقدار کمتر بهتر است" : info.direction === "contextual" ? "اهمیت آن به Hero بستگی دارد" : "مقدار بیشتر بهتر است";
  return <article className={`benchmark-cell is-${tone(entry)}`} title={info.description}><header><span className="benchmark-label" lang="en" dir="ltr">{info.label}<span className="analysis-metric-help" tabIndex={0} aria-label={info.description}><Info /><span role="tooltip">{info.description}</span></span></span><strong className="benchmark-raw" lang="en" dir="ltr">{entry.formattedValue}</strong></header><p>{info.description}</p><div className="benchmark-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={entry.qualityPercentile}><span style={{ width: `${entry.qualityPercentile}%` }} /></div><footer><b>بالاتر از {fa.format(entry.qualityPercentile)}٪</b><small>{direction}</small></footer></article>;
}
...
function TimelineView({ analysis, player, minute, setMinute }: { analysis: MatchAnalysis; player: MatchPlayerAnalysis; minute: number; setMinute: (value: number) => void }) {
  ...
  return <div className={`timeline-view ${info.className}`} dir="rtl">
    <header className="timeline-heading"><div className="timeline-player-heading"><img src={portrait(player)} alt=""/><span><small lang="en" dir="ltr">PLAYER PROGRESSION</small><h4 lang="en" dir="ltr">{player.heroName}</h4><p>{info.description} در طول Match</p></span><img className="timeline-position-icon" src={positionIcon(player.position)} alt=""/></div><div className="timeline-toolbar"><div className="timeline-metric-switch">{(Object.keys(TIMELINE) as TimelineMetric[]).map((key) => <button className={metricKey === key ? "is-active" : ""} type="button" key={key} onClick={() => setMetricKey(key)} lang="en" dir="ltr">{TIMELINE[key].label}</button>)}</div><div className="timeline-scope-switch"><button className={scope === "solo" ? "is-active" : ""} type="button" onClick={() => setScope("solo")}>فقط این Hero</button><button className={scope === "role" ? "is-active" : ""} type="button" onClick={() => setScope("role")} disabled={!opponent}>مقایسه هم‌Position</button><button className={scope === "all" ? "is-active" : ""} type="button" onClick={() => setScope("all")}>هر ۱۰ Hero</button></div></div></header>
...
function ProgressionMilestones({player,opponent,metricKey,duration}:{player:MatchPlayerAnalysis;opponent?:MatchPlayerAnalysis;metricKey:TimelineMetric;duration:number}){const field=metricKey==="gold"?"goldDelta":metricKey==="xp"?"xpDelta":"lastHitDelta";return <section className="progression-milestones" dir="ltr"><header><div><Activity/><span><b>{TIMELINE[metricKey].label} MILESTONES</b><small>ایستگاه‌های مهم بازی در دقیقه‌های ۵، ۱۰، ۲۰، ۳۰، ۴۰ و ۶۰</small></span></div>{opponent&&<span className="milestone-rival"><img src={portrait(opponent)} alt=""/>VS {opponent.heroName}</span>}</header><div>{MILESTONES.map((minute)=>{const point=minute<=duration?closest(player.timeline,minute):undefined,rival=opponent&&minute<=duration?closest(opponent.timeline,minute):undefined,value=point?.[metricKey]??null,rivalValue=rival?.[metricKey]??null,delta=value!==null&&rivalValue!==null?value-rivalValue:null,cohort=player.cohort?.milestones.find((entry)=>entry.minute===minute),cohortValue=cohort?.[metricKey]??null,cohortDelta=value!==null&&cohortValue!==null?value-cohortValue:null,trend=snapshotTrend(player.timeline,point,field);return <article key={minute} className={`is-${trend}${minute>duration?" is-disabled":""}`}><header><span>MINUTE</span><b>{minute}</b></header><strong>{value===null?"—":en.format(value)}</strong><div>{opponent&&<small>{delta===null?"مقایسه حریف موجود نیست":`${delta>0?"+":""}${en.format(delta)} نسبت به حریف`}</small>}{cohort&&<small>{cohortDelta===null?"میانه آماری موجود نیست":`${cohortDelta>0?"+":""}${en.format(cohortDelta)} نسبت به میانه`}</small>}</div><TrendIcon trend={trend}/></article>})}</div></section>}
