...
export default function MatchAnalysisPanel({ match, active, onPositionOverrides }: { match: Match; active: boolean; onPositionOverrides?: (updates:Record<string,number>) => void }) {
  ...
  return <section className="match-analysis">
    ...
    <div className="analysis-player-dock"><PlayerStrip players={analysis.players} selected={player.playerSlot} select={setSlot} />{onPositionOverrides&&<PositionSwapMenu player={player} players={analysis.players} confirm={confirmPositions}/>}</div>
...
function PositionSwapMenu({player,players,confirm}:{player:MatchPlayerAnalysis;players:MatchPlayerAnalysis[];confirm:(updates:Record<string,number>)=>void}) {
...
function Summary({ player,players,duration,confirmPositions }: { player: MatchPlayerAnalysis;players:MatchPlayerAnalysis[];duration:number;confirmPositions?: (updates:Record<string,number>)=>void }) {
  ...
  return <div className="analysis-summary">{player.positionResolution?.roleSwapDetected&&<RoleSwapReview player={player} partner={swapPartner} confirm={confirmPositions}/>}<DomainProfile player={player} duration={duration}/><div className="analysis-overview-grid">
    <section className="benchmark-board"><div className="benchmark-board-heading"><SectionHeading icon={<BarChart3 />} title="Benchmark Spectrum" detail={player.benchmarkSource === "hero" || player.benchmarkSource === "cohort" ? "مقایسه با داده عمومی همین Hero" : player.benchmarkSource === "match" ? "مقایسه داخلی بین بازیکنان همین Match" : "داده کافی نیست"} /><CohortBadge player={player}/></div>{player.benchmarks.length ? <div className="benchmark-grid">{player.benchmarks.map((entry) => <MetricCard entry={entry} key={entry.key} />)}</div> : <p>برای این Match، Benchmark قابل اتکا موجود نیست.</p>}</section>
...
function CohortBadge({player}:{player:MatchPlayerAnalysis}){const cohort=player.cohort;return <details className="analysis-cohort-badge"><summary><Database/>مرجع تحلیل<Info/></summary><div>{cohort?<><strong>{cohort.label}</strong><p>Hero + Pos: {cohort.heroPositionSamples.toLocaleString("fa-IR")} Match · Role Share: {cohort.positionPickRate==null?"—":`${cohort.positionPickRate.toLocaleString("fa-IR")}%`}</p><small>منبع Meta: STRATZ · Benchmark: OpenDota snapshot</small>{cohort.limitations.slice(0,2).map((item)=><small key={item}>{item}</small>)}</>:<p>Snapshot عمومی هنوز آماده نیست؛ داده کاربران سایت وارد Benchmark نمی‌شود.</p>}</div></details>}
...
function DomainProfile({player,duration}:{player:MatchPlayerAnalysis;duration:number}){const domains=calculatePerformanceDomains(player.benchmarks,duration,player.position);return <section className="performance-domain-profile"><header><ScoreRing score={playerScore(player,duration)}/></header><div>{domains.map((item)=><article key={item.key} className={`is-${item.score==null?"unavailable":performanceTone(item.score)}`}><span><b>{item.label}</b><small>{item.metricCount?`${item.metricCount} معیار قابل محاسبه`:"داده کافی نیست"}</small></span><strong>{item.score??"—"}</strong><i><em style={{width:`${item.score||0}%`}}/></i></article>)}</div></section>}
...
function MetricCard({ entry }: { entry: MatchBenchmarkMetric }) {
  ...
  return <article className={`benchmark-cell is-${tone(entry)}`} title={info.description}><header><span className="benchmark-label" lang="en" dir="ltr">{info.label}<span className="analysis-metric-help" tabIndex={0} aria-label={info.description}><Info /><span role="tooltip">{info.description}</span></span></span><strong className="benchmark-raw" lang="en" dir="ltr">{entry.formattedValue}</strong></header><div className="benchmark-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={entry.qualityPercentile}><span style={{ width: `${entry.qualityPercentile}%` }} /></div><footer><b>بهتر از {fa.format(entry.qualityPercentile)}٪</b><small>{context} · {direction}</small></footer></article>;
}
