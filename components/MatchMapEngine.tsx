...
function UtilityFacts({ player }: { player: MatchPlayerAnalysis }) {
  ...
  return <aside className="map-insight-panel is-wide">
    ...
    <div className="map-fact-grid"><Metric label="Observer placed" value={data?.observersPlaced} icon="/items/ward_observer.png" /><Metric label="Sentry placed" value={data?.sentriesPlaced} icon="/items/ward_sentry.png" /><Metric label="Observer deward" value={data?.observersDestroyed} icon="/items/ward_observer.png" /><Metric label="Sentry deward" value={data?.sentriesDestroyed} icon="/items/ward_sentry.png" /><Metric label="Observer avg life" value={data?.averageObserverLifetimeSeconds} suffix="s" icon="/items/ward_observer.png" /><Metric label="Sentry avg life" value={data?.averageSentryLifetimeSeconds} suffix="s" icon="/items/ward_sentry.png" /><Metric label="Observer ≥300s" value={data?.observersLastingAtLeast300Seconds} icon="/items/ward_observer.png" /><Metric label="Productive sentry" value={data?.productiveSentriesEstimate} icon="/items/ward_sentry.png" /><Metric label="Lane resources" value={data?.laneResourcePurchases} icon="/items/tango.png" /><Metric label="Dust uses" value={data?.dustUses} icon="/items/dust.png" /><Metric label="Smoke uses" value={data?.smokeUses} icon="/items/smoke_of_deceit.png" /><Metric label="Kills under Smoke" value={data?.smokeKillParticipations} icon="/items/smoke_of_deceit.png" /><Metric label="Gem purchases" value={data?.gemPurchases} icon="/items/gem.png" /><Metric label="Stacks" value={data?.campsStacked} icon="/map-analysis/camp.png" /></div>
    ...
    {Boolean(player.ownershipEvents?.length)&&<section className="ownership-timeline"><header><ShieldCheck/><div><b>Gem / Divine ownership</b><small>خرید و دارنده نهایی؛ بدون حدس زمان انتقال</small></div></header>{player.ownershipEvents?.map((event)=><article key={`${event.item}-${event.purchaserPlayerSlot}`}><img src={event.item==="gem"?"/items/gem.png":"/items/rapier.png"} alt=""/><div><b>{event.item==="gem"?"Gem of True Sight":"Divine Rapier"}</b><small>{event.transfer==="enemy"?"در پایان دست تیم حریف":event.transfer==="ally"?"در پایان دست هم‌تیمی":event.transfer==="none"?"در پایان نزد خریدار":"انتقال قابل اثبات نیست"}</small></div><span>{event.confidence==="high"?"اطمینان بالا":event.confidence==="medium"?"اطمینان متوسط":"اطمینان محدود"}</span></article>)}</section>}
    <Note text={data?.note} />
  </aside>;
}
